...
- Current verification baseline: `swift test` with 378 executed tests passing; 34 performance benchmarks are opt-in and skipped by default
...
- portable in-process `git` support now runs on the active `BashFilesystem`, including the default iPhone-compatible `VirtualFileSystem`; coverage includes local repo lifecycle, status, bare-remote clone/push, GitHub HTTPS `ls-remote`/clone/push transport, and credential-store plumbing
