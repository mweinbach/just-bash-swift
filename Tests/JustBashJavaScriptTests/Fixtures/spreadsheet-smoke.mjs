...
const preview = await workbook.render({ sheetName: "Dashboard", range: "A1:L14", scale: 1, previewMode: "schematic", chartPreview: "omit" });
if (preview.mime !== "image/png" || (await preview.arrayBuffer()).length < 100) {
  throw new Error("worksheet render failed");
}
await preview.save("./output/dashboard.png");

const xlsx = await SpreadsheetFile.exportXlsx(workbook);
await xlsx.save("./output/dashboard.xlsx");
...
await fs.mkdir("./external-xlsx/_rels", { recursive: true });
await fs.mkdir("./external-xlsx/xl/_rels", { recursive: true });
await fs.mkdir("./external-xlsx/xl/worksheets", { recursive: true });
await fs.writeFile("./external-xlsx/[Content_Types].xml", `<?xml version="1.0" encoding="UTF-8"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/></Types>`);
await fs.writeFile("./external-xlsx/_rels/.rels", `<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>`);
await fs.writeFile("./external-xlsx/xl/workbook.xml", `<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Compressed" sheetId="1" r:id="rId1"/></sheets></workbook>`);
await fs.writeFile("./external-xlsx/xl/_rels/workbook.xml.rels", `<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/></Relationships>`);
await fs.writeFile("./external-xlsx/xl/worksheets/sheet1.xml", `<?xml version="1.0" encoding="UTF-8"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetData><row r="1"><c r="A1" t="inlineStr"><is><t>compressed import</t></is></c></row></sheetData></worksheet>`);
execSync("cd ./external-xlsx && zip -q -r ../external-compressed.xlsx '[Content_Types].xml' _rels xl");
const compressedWorkbook = await SpreadsheetFile.importXlsx(await FileBlob.load("./external-compressed.xlsx"));
